...
func Start(addr string, serverType int) {
	if serverType == ServerHttps {
		startHttpsServer(addr)
	} else if serverType == Server403 {

	} else if serverType == ServerActuator {
		startHttpActuatorServer(addr)
	} else {
...
func helloHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Server", "PHP/8.3")
	io.WriteString(w, `<!DOCTYPE html><html><header><title>在线资产管理平台</title></header><body> appid":"skylar6 360新天擎 正在跳转中...</body></html>`)
}
